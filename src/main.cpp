...
std::tuple<Serial, int> json_file_parse(const std::string &filepath)
{
    ...
    if (!parse_ok)
    {
        std::cerr << "Error: parse json failed -> " << reader.getFormattedErrorMessages() << std::endl;
        ifs.close();
        return std::make_tuple(ser, camera_index);
    }
...
std::tuple<Serial, int> parameter_parse(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            std::cout << "Usage: ./uav_controller [--port PORT] [--baudrate BAUDRATE] [--bytesize BYTESIZE] [--parity PARITY] [--stopbits STOPBITS]" << std::endl;
            std::cout << "Default: --port /dev/ttyUSB0 --baudrate 115200 --bytesize 8 --parity N --stopbits 1" << std::endl;
            exit(0);
        }
...
int main(int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        else
        {
            ...
            LineFollower LF(lock, camera_index, ser.Port, ser.BaudRate, ser.ByteSize, ser.Parity, ser.Stopbits);
            LF.run();
        }
    }
    else
    {
        ...
        printf("Stop Bits: %u\n", args.Stopbits);
        printf("Camera Index: %d\n", index);
        LineFollower LF(lock, index, args.Port, args.BaudRate, args.ByteSize, args.Parity, args.Stopbits);
        LF.run();
    }
}
