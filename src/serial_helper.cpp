...
#include <set>
#include <iomanip>
SerialHelper::SerialHelper(std::mutex &lock, std::string Port, uint32_t BaudRate, uint8_t ByteSize, char Parity, uint8_t Stopbits) : lock(lock), port(Port)
{
    switch (BaudRate)
    {
    case 9600:
...
{
    std::thread connect_thread([this, func]()
...
/*
std::vector<std::string> SerialHelper::find_usb_tty(uint16_t vendor_id, uint16_t product_id)
{
...
{
    std::thread tDataReceived([this, &func]()
            ...
            if (_is_connected)
            {
                try
                {
                    ...
                    if (_serial.IsDataAvailable())
                    {
                        ...
                        {
                            char byte;
                            _serial.ReadByte(byte, 20); // ReadByte 会阻塞，如果未设置 timeout，请设置
                            data.push_back(static_cast<uint8_t>(byte));
                        }

                        if (!data.empty() && data.size() >= 9)
                        {
                            uint32_t height = (static_cast<uint32_t>(data[5]) & 0xFF) |
