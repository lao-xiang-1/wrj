...
void LineFollower::line(const cv::Mat &image, const cv::Point &center,
  ...
  if (is_follow && is_begin == 1) {
    ...
    // PID计算横移速度
    double speed_row = round(pid_r.pid_calculate(
        0.1, 0, 5 * fuhao(offset_center_d), offset_center_d));

    // PID计算偏航速度
    double speed_yaw = 0;
    if (cont_cent[0].x != 0 && cont_cent[1].x != 0) {
      speed_yaw = round(
          pid_t.pid_calculate(0.1, 0, 5 * fuhao(line_angle1), line_angle1));
    } else if (cont_cent[1].x != 0 && cont_cent[2].x != 0) {
      speed_yaw = round(
          pid_t.pid_calculate(0.1, 0, 5 * fuhao(line_angle2), line_angle2));
    } else if (cont_cent[2].x != 0 && cont_cent[3].x != 0) {
      speed_yaw = round(
          pid_t.pid_calculate(0.1, 0, 5 * fuhao(line_angle3), line_angle3));
    }
    ...
    if (abs(offset_center_d) < offset_c && cnt >= 2) {
      ...
      std::cout << "[LINE] 前进: " << speed_line << ", 偏航修正: " << speed_yaw
                << std::endl;
    } else {
      // 偏移较大：横移修正
