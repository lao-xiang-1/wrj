...
// ---------------- 构造函数实现 ----------------
LineFollower::LineFollower(std::mutex &lock, int index, std::string Port, uint32_t BaudRate, uint8_t ByteSize, char Parity, uint8_t Stopbits)
    : scan_content(""), is_code_center(false), is_follow(true), is_forward(false),
      is_offset(false), is_decode(false), offset_o(0), is_begin(1), get_red(false),
      get_qr(false), qr_count(0), velocity_sum(0.0), last_filter_velocity(0.0),
      V_kp(0.0), V_ki(0.0), Kp_row(0.0), Kd_row(0.0), Kp_yaw(0.0), Kd_yaw(0.0),
      timing(0.0), speed_yaw_global(0.0), offset_center_d(0), lost5(0), cnt(0), out(0),
      index(index), airplanceApi(lock, Port, BaudRate, ByteSize, Parity, Stopbits)
{
}
...
// ---------------- 私有成员函数实现 ----------------
cv::Point LineFollower::calculate_centroid(const cv::Mat &image, int kernel_size, int threshold)
{
...
cv::Mat LineFollower::detect_red_by_rgb_diff(const cv::Mat &image, int threshold, int min_red)
{
    cv::Mat img_float;
    ...
    // 生成红色区域掩膜
    cv::Mat mask = ((diff_rg > threshold) & (diff_rb > threshold) & (r > min_red));
...
cv::Point LineFollower::get_contour_center(const std::vector<cv::Point> &contour)
{
...
std::pair<std::vector<cv::Mat>, std::vector<cv::Point>> LineFollower::slice_out(const cv::Mat &im, int num)
{
...
void LineFollower::decode(const cv::Mat &image)
{
    ...
    for (auto symbol = zbar_image.symbol_begin(); symbol != zbar_image.symbol_end(); ++symbol)
    {
        ...
        std::cout << "[QR] 偏移量: x=" << offset_x << ", y=" << offset_y << std::endl;
        is_forward = false;
    ...
    // 输出调试信息
    std::cout << "[QR] 识别计数: " << qr_count << ", 丢失计数: " << lost5 << std::endl;
    auto t1 = std::chrono::high_resolution_clock::now();
    double qr_time = std::chrono::duration<double>(t1 - t0).count();
    std::cout << "[QR] 识别耗时: " << qr_time << "s" << std::endl;
}

void LineFollower::line(const cv::Mat &image, const cv::Point &center, const std::vector<cv::Point> &cont_cent)
{
    // 绘制分割线（可视化）
    cv::line(image, cv::Point(0, 65), cv::Point(480, 65), cv::Scalar(30, 30, 30), 1);
    cv::line(image, cv::Point(0, 130), cv::Point(480, 130), cv::Scalar(30, 30, 30), 1);
    cv::line(image, cv::Point(0, 195), cv::Point(480, 195), cv::Scalar(30, 30, 30), 1);
    cv::line(image, cv::Point(240, 0), cv::Point(240, 260), cv::Scalar(30, 30, 30), 1);

    // 计算各块轮廓中心的偏移角度
    int line_angle1 = 0, line_angle2 = 0, line_angle3 = 0;
    if (cont_cent[0].x != 0 && cont_cent[1].x != 0)
    {
        line_angle1 = static_cast<int>(180 * atan2(cont_cent[1].x - cont_cent[0].x, cont_cent[1].y + 65 - cont_cent[0].y) / M_PI);
    }
    if (cont_cent[1].x != 0 && cont_cent[2].x != 0)
    {
        line_angle2 = static_cast<int>(180 * atan2(cont_cent[2].x - cont_cent[1].x, cont_cent[2].y + 130 - cont_cent[1].y - 65) / M_PI);
    }
    if (cont_cent[2].x != 0 && cont_cent[3].x != 0)
    {
        line_angle3 = static_cast<int>(180 * atan2(cont_cent[3].x - cont_cent[2].x, cont_cent[3].y + 195 - cont_cent[2].y - 130) / M_PI);
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        if (cont_cent[i].x != 0)
        {
            offset_center_d += cont_cent[i].x - 240;
            cnt++;
        }
    }
    if (cnt != 0)
    {
        ...
        {
            is_begin = 0;
            offset_o = offset_center_d;
        }
    }
    else
    {
        offset_center_d = offset_o;
    }
    ...
    // ---------------- 巡线控制逻辑 ----------------
    if (is_follow && is_begin == 1)
    {
        // 初始阶段：向前飞行
        out++;
        if (out < 45)
        {
            airplanceApi.move_forward(120);
            std::cout << "[LINE] 初始前进" << std::endl;
        }
        else
        {
            airplanceApi.move(80, 0, 0, 0);
            if (cnt >= 1)
            {
                is_begin = 0;
            }
    ...
    {
        // PID计算横移速度
        double speed_row = round(pid_r.pid_calculate(0.1, 0, 5 * fuhao(offset_center_d), offset_center_d));

        // PID计算偏航速度
        double speed_yaw = 0;
        if (cont_cent[0].x != 0 && cont_cent[1].x != 0)
        {
            speed_yaw = round(pid_t.pid_calculate(0.1, 0, 5 * fuhao(line_angle1), line_angle1));
        }
        else if (cont_cent[1].x != 0 && cont_cent[2].x != 0)
        {
            speed_yaw = round(pid_t.pid_calculate(0.1, 0, 5 * fuhao(line_angle2), line_angle2));
        }
        else if (cont_cent[2].x != 0 && cont_cent[3].x != 0)
        {
            speed_yaw = round(pid_t.pid_calculate(0.1, 0, 5 * fuhao(line_angle3), line_angle3));
        }
        speed_yaw_global = speed_yaw;
        ...
        if (is_decode)
        {
            ...
            else if (abs(speed_yaw) < 60)
            {
                speed_yaw = static_cast<int>(speed_yaw * 0.7 + fuhao(speed_yaw) * 38);
            }
            if (abs(speed_yaw) > 80)
            {
                speed_yaw = 0;
        ...
        // 偏航速度限幅
        speed_yaw = clamp(speed_yaw, -250.0, 250.0);

        // 红色圆点识别：调整偏移阈值
        int offset_c = get_red ? 40 : 45;

        // 巡线控制决策
        if (abs(offset_center_d) < offset_c && cnt >= 2)
        {
            // 偏移较小：向前+偏航修正
            airplanceApi.move(0, speed_line, 0, static_cast<int>(speed_yaw));
            std::cout << "[LINE] 前进: " << speed_line << ", 偏航修正: " << speed_yaw << std::endl;
        }
        else
        {
            // 偏移较大：横移修正
            speed_row = static_cast<int>(offset_center_d * 0.9);
            speed_row = clamp(speed_row, -200.0, 200.0);
            airplanceApi.move(speed_row, 0, 0, 0);
            std::cout << "[LINE] 横移修正: " << speed_row << std::endl;
        }
        std::cout << "-----------------------------------------------" << std::endl;
    }
}
...
void LineFollower::start_video()
{
    ...
    for (;;)
    {
        ...
        {
            // 找最大红色轮廓
            auto cnt = *std::max_element(cnts.begin(), cnts.end(),
                                         [](const std::vector<cv::Point> &a, const std::vector<cv::Point> &b)
                                         {
                                             return cv::contourArea(a) < cv::contourArea(b);
                                         });
            ...
            if (radius > 20 && center_red.y > 0 && center_red.y < 250)
            {
                ...
                cv::circle(red_img, center_red, static_cast<int>(radius), cv::Scalar(0, 250, 0), -1);
                get_red = true;
