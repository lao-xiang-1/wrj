...
void UPUavControl::send_msg() {
  while (_active) {
    ...
    
    {
        ...
        if (_isConn && !msg_list.empty()) {
            data_to_send = msg_list.front();
            msg_list.pop();
    ...
    if (has_data_to_send) {
        ...
        DATA_LOG(data_to_send);
    }
#ifdef DEBUG
    else if (!_isConn) {
      std::queue<Data> msg_copy;
      {
          std::lock_guard<std::mutex> gard(lock);
          if (!msg_list.empty()) {
              msg_copy = msg_list;
...
void UPUavControl::move_left(int16_t speed) { setMoveAction(0, 0, -speed, 0); }
void UPUavControl::move_right(int16_t speed) { setMoveAction(0, 0, speed, 0); }
