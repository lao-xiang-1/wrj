...
#define DATA_LOG(data)
#endif
UPUavControl::UPUavControl(std::mutex &lock, std::string Port, uint32_t BaudRate, uint8_t ByteSize, char Parity, uint8_t Stopbits) : lock(lock), ser(lock, Port, BaudRate, ByteSize, Parity, Stopbits)
{
}
...
std::tuple<std::vector<uint8_t>, uint16_t> UPUavControl::generateCmd(uint8_t device, uint8_t cmd, uint8_t len, Data data)
{
    Data buffer(len + 6, 0);
    buffer[0] = 0xF5;
    buffer[1] = 0x5F;
...
void UPUavControl::move_left(int16_t speed)
{
    setMoveAction(0, 0, -speed, 0);
}
void UPUavControl::move_right(int16_t speed)
{
    setMoveAction(0, 0, speed, 0);
}
void UPUavControl::move_up(int16_t speed)
{
    setMoveAction(0, 0, speed, 0);
}
void UPUavControl::move_down(int16_t speed)
{
    setMoveAction(0, 0, speed, 0);
}
...
void UPUavControl::turn_left(int16_t speed)
{
    setMoveAction(0, 0, 0, speed);
}
void UPUavControl::turn_right(int16_t speed)
{
    setMoveAction(0, 0, 0, speed);
}
void UPUavControl::unlock()
{
    setMoveAction(-500, -500, -500, 500);
}
...
void UPUavControl::on_data_received(Data &data)
{
    ...
    {
        uint32_t height = ((data[5] & 0xFF) | ((data[6] & 0xFF) << 8) | ((data[7] & 0xFF) << 16) | ((data[8] & 0xFF) << 24));
