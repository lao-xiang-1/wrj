...
public:
    ...
    UPUavControl(std::mutex &lock, std::string Port = "/dev/ttyUSB0", uint32_t BaudRate = 115200, uint8_t ByteSize = 8, char Parity = 'N', uint8_t Stopbits = 1);
    ~UPUavControl() noexcept;
    void myserial_on_connected_changed(bool is_connected);
    // void write(Data data);
    void send_msg();
    std::tuple<Data, uint16_t> generateCmd(uint8_t device, uint8_t cmd, uint8_t len, Data data);
