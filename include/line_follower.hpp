...
private:
     ...
     */
    cv::Point calculate_centroid(const cv::Mat &image, int kernel_size = 5, int threshold = 127);
     ...
     * @return 红色区域二值掩膜
     */
    cv::Mat detect_red_by_rgb_diff(const cv::Mat &image, int threshold = 40, int min_red = 80);
     ...
     */
    std::pair<std::vector<cv::Mat>, std::vector<cv::Point>> slice_out(const cv::Mat &im, int num);
     ...
     */
    void line(const cv::Mat &image, const cv::Point &center, const std::vector<cv::Point> &cont_cent);
...
public:
     ...
     */
    LineFollower(std::mutex &lock, int index = 0,std::string Port = "/dev/ttyUSB0", uint32_t BaudRate = 115200, uint8_t ByteSize = 8, char Parity = 'N', uint8_t Stopbits = 1);
