...
public:
    ...
    Data receive_data;
    SerialHelper(std::mutex &lock, std::string Port = "/dev/ttyUSB0", uint32_t BaudRate = 115200, uint8_t ByteSize = 8, char Parity = 'N', uint8_t Stopbits = 1);
...
private:
    ...
    // std::vector<std::string> find_usb_tty(uint16_t vendor_id = 0, uint16_t product_id = 0);
};
